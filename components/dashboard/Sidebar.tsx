...
export function Sidebar({ isOpen }: SidebarProps) {
  return (
    <aside className="hidden md:flex fixed left-0 top-0 h-screen w-64 flex-col glass-card border-r border-white/40 shadow-xl shadow-black/5">
          ...
          {menuItems.map((item, index) => (
            <button
              ...
              className={`group w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 ${
                item.active
                  ? "bg-white/30 border border-white/40 text-primary font-medium shadow-sm"
