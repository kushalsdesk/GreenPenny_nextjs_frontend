...
export function AccountCards() {
  ...
  const handleAddAccount = (accountData: {
    ...
    const newAccount: AccountCardProps = {
      ...
      name: accountData.nickname || `${accountData.accountType} Account`,
      balance: parseFloat(accountData.balance).toLocaleString("en-US", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      }),
      currency: "USD",
      type: accounts.length === 0 ? "primary" : accounts.length === 1 ? "secondary" : "tertiary",
    };
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
        ...
        {/* Add New Account Card - Only show if less than 3 accounts */}
        {accounts.length < 3 && (
          <button
            onClick={() => setIsModalOpen(true)}
            className="glass-card-glow p-4 sm:p-6 border-2 border-dashed border-primary/40 rounded-2xl hover:border-primary/60 hover:bg-white/30 hover:scale-[1.02] transition-all duration-300 h-full min-h-[280px] flex flex-col items-center justify-center gap-4 group"
            aria-label="Add new account"
          >
            <div className="p-4 sm:p-5 bg-white/30 backdrop-blur-md rounded-xl border border-primary/30 group-hover:bg-white/40 group-hover:border-primary/50 group-hover:scale-110 transition-all duration-300">
              <svg
                width="40"
                height="40"
                viewBox="0 0 24 24"
